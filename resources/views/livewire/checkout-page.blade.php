    ...
    <div class="md:col-span-12 lg:col-span-8 col-span-12">
      ...
      <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
        ...
        <ul class="grid w-full gap-6 md:grid-cols-2">
          <li>
            <input class="hidden peer" wire:model="payment_method" id="cod" value="cod" type="radio" />
            <label for="cod"
                   class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer dark:border-gray-700 peer-checked:border-blue-600 peer-checked:text-blue-600 hover:bg-gray-100 dark:bg-gray-800">
              <span class="w-full text-lg font-semibold">Bayar di Tempat</span>
            </label>
          </li>
          <li>
            <input class="hidden peer" wire:model="payment_method" id="transfer" value="transfer" type="radio" />
            <label for="transfer"
                   class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer dark:border-gray-700 peer-checked:border-blue-600 peer-checked:text-blue-600 hover:bg-gray-100 dark:bg-gray-800">
    ...
    <div class="md:col-span-12 lg:col-span-4 col-span-12">
      ...
      @php
        ...
        foreach($cart_items as $item) {
          $nota .= "- {$item['name']} ({$item['quantity']}x) = Rp".number_format($item['total_amount'],0,',','.')."\n";
        }
            ...
            <li class="py-3 sm:py-4" wire:key="{{$item['product_id']}}">
              <div class="flex items-center">
                <div class="flex-shrink-0">
                  <img src="{{url('storage', $item['image'])}}" alt="{{$item['name']}}" class="w-12 h-12 rounded-full">
                </div>
