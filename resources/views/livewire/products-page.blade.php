    ...
    <div class="mb-6 rounded-xl bg-gradient-to-r from-amber-100 to-orange-50 border border-amber-200 px-4 py-4 sm:py-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                <!-- Sort -->
                {{-- Toolbar kanan: Search + Sort + Reset --}}
                <div class="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div class="flex items-center gap-3">
                ...
                <!-- (Opsional) Field cari nama produk kalau kamu punya property Livewire: search -->
                {{-- <div class="relative">
          <input type="text" wire:model.live="search" placeholder="Cari produk…"
                 class="w-56 bg-white border border-amber-200 rounded-lg px-3 py-2 pl-9 focus:outline-none focus:ring-2 focus:ring-amber-400 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-200">
          <svg class="absolute left-2.5 top-2.5 w-4 h-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none"
               ...
               d="M21 21l-4.35-4.35M10 18a8 8 0 100-16 8 8 0 000 16z"/></svg>
                ...
                <!-- Reset Filter (aman ke /products kalau belum ada method reset) -->
                <a href="/products"
            ...
            <div class="flex flex-wrap gap-6 lg:gap-8 mb-10">
                ...
                <aside class="w-full lg:w-1/4 shrink-0">
                    <div class="sticky top-6 space-y-5">
                        ...
                        <!-- Categories -->
                        <div
                            class="p-5 bg-white border border-amber-200 rounded-2xl shadow-sm dark:bg-gray-900 dark:border-gray-800">
                                ...
                                @foreach ($categories as $category)
                                    <li wire:key="cat-{{ $category->id }}">
                                        <label for="cat-{{ $category->slug }}"
                                            ...
                                            <span
                                                class="text-base text-gray-700 group-hover:text-amber-700 dark:text-gray-300 dark:group-hover:text-gray-100">
                                                {{ $category->name }}
                                            </span>
                                        </label>
                                    </li>
                        ...
                        <div
                            ...
                            <div class="mt-5">
                                <input type="range" wire:model.live="price_range" max="5000000" step="100000"
                                    class="w-full h-2 bg-amber-100 rounded-lg appearance-none cursor-pointer">
                                <div class="mt-3 flex items-center justify-between text-sm">
                                    <span
                                        class="inline-flex items-center gap-1 font-semibold text-amber-700 bg-amber-50 px-2 py-1 rounded">
                                        {{ Number::currency($price_range, 'IDR') }}
                                    </span>
                                    <span
                                        class="inline-flex items-center gap-1 font-semibold text-amber-700 bg-amber-50 px-2 py-1 rounded">
                                        {{ Number::currency(5000000, 'IDR') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                <main class="min-w-0 flex-1 lg:w-3/4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 md:gap-6">
                        ...
                        @foreach ($products as $product)
                            <div livewire:key="prod-{{ $product->id }}"
                                ...
                                <div class="absolute z-10 left-3 top-3 flex flex-col gap-2">
                                    @isset($product->on_sale)
                                        @if ($product->on_sale || (!empty($onSale) && $onSale))
                                            <span
                                                class="inline-flex items-center text-[11px] font-semibold px-2 py-1 rounded-full bg-rose-100 text-rose-700">
                                                Diskon
                                            </span>
                                        @endif
                                    @endisset
                                    @isset($product->featured)
                                        @if ($product->featured || (!empty($featured) && $featured))
                                            <span
                                                class="inline-flex items-center text-[11px] font-semibold px-2 py-1 rounded-full bg-amber-100 text-amber-700">
                                                Unggulan
                                            </span>
                                        @endif
                                    @endisset
                                ...
                                <div class="p-4">
                                    <div class="flex items-start justify-between gap-3">
                                        <h3
                                            class="text-lg font-semibold text-gray-800 leading-snug dark:text-gray-200">
                                            {{ $product->name }}
                                        </h3>
                                    </div>
                                            ...
                                            @isset($product->compare_at_price)
                                                @if ($product->compare_at_price > $product->price)
                                                    <span class="ml-2 text-sm line-through text-gray-400">
                                                        {{ Number::currency($product->compare_at_price, 'IDR') }}
                                                    </span>
                                                @endif
                                            @endisset
                                        </p>
                                ...
                                <div class="px-4 pb-4">
                                    <button wire:click.prevent="addToCart({{ $product->id }})"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="currentColor"
                                            viewBox="0 0 16 16">
                                            <path
                                                d="M0 1.5A.5.5 0 0 1 .5 1H2l.5 2H14.5a.5.5 0 0 1 .49.598l-1 5a.5.5 0 0 1-.465.401l-9.4.472L4.4 11H13a.5.5 0 0 1 0 1H4a.5.5 0 0 1-.49-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4z" />
                                        </svg>
                                        <span wire:loading.remove wire:target="addToCart({{ $product->id }})">Tambah
                                            ke Keranjang</span>
                                        <span wire:loading
                    ...
                    @if (
                        ...
                        <div
                            class="mt-10 text-center bg-white border border-amber-200 rounded-2xl p-10 dark:bg-gray-900 dark:border-gray-800">
                            <p class="text-gray-600 dark:text-gray-300">Produk tidak ditemukan. Coba ubah filter atau
                                kata kunci.</p>
                        </div>
                    @endif
