<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
  ...
  <!-- Header -->
  <div class="mb-8 rounded-xl bg-gradient-to-r from-amber-100 to-orange-50 border border-amber-200 px-5 py-6">
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
      ...
      {{-- <div class="relative">
        <input type="text" wire:model.live="categorySearch" placeholder="Cari kategori…"
               class="w-64 bg-white border border-amber-200 rounded-lg px-3 py-2 pl-9 focus:outline-none focus:ring-2 focus:ring-amber-400 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-200">
  ...
  <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
    ...
    @foreach ($categories as $category)
      <a wire:key="cat-{{ $category->id }}" wire:navigate
         ...
         class="group relative flex flex-col rounded-2xl overflow-hidden bg-white border border-amber-200 hover:shadow-md transition dark:bg-slate-900 dark:border-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
        ...
        <div class="p-4 md:p-5">
          <div class="flex items-start justify-between gap-3">
            <h3 class="text-lg md:text-xl font-semibold text-gray-800 leading-snug group-hover:text-amber-700 dark:text-gray-200 dark:group-hover:text-gray-100">
              {{ $category->name }}
            </h3>

            <!-- Icon panah -->
            <span class="shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-full bg-amber-50 text-amber-700 border border-amber-200 group-hover:bg-amber-100">
          ...
          @if(isset($category->products_count))
            <div class="mt-2">
              <span class="inline-flex items-center text-xs font-semibold px-2 py-1 rounded-full bg-amber-100 text-amber-700">
