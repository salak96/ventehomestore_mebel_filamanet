                        ...
                        <tbody class="divide-y divide-amber-100">
                            @forelse ($cart_items as $item)
                                <tr wire:key="row-{{ $item['product_id'] }}" class="align-top">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                @isset($item['variant'])
                                                    <div class="mt-1 text-xs text-gray-500">Varian: {{ $item['variant'] }}
                                                    </div>
                                                @endisset
                                            </div>
                                        </div>
                                    ...
                                    <td class="px-6 py-4">
                                        <div
                                            class="inline-flex items-center rounded-xl border border-amber-200 overflow-hidden">
                                            <button wire:click="decreaseItem({{ $item['product_id'] }})"
                                                class="px-3 py-2 hover:bg-amber-50" aria-label="Kurangi jumlah">
                                                −
                                            </button>
                                            <span
                                                class="px-4 py-2 min-w-8 text-center font-medium">{{ $item['quantity'] }}</span>
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-16">
                                        <div class="text-center">
                                            <div
                                                class="mx-auto w-16 h-16 rounded-full bg-amber-50 flex items-center justify-center mb-4">
                                                🛒
                                            </div>
                            ...
                            <div class="flex gap-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="mt-1 text-sm text-gray-600">
                                        Harga: <span
                                            class="font-medium text-gray-800">{{ Number::currency($item['unit_amount'], 'IDR') }}</span>
                                    </div>
                                    <div class="mt-3 flex items-center justify-between">
                                        <div
                                            class="inline-flex items-center rounded-xl border border-amber-200 overflow-hidden">
                                            <button wire:click="decreaseItem({{ $item['product_id'] }})"
                                                class="px-3 py-1.5 hover:bg-amber-50">−</button>
                                            <span
                                                class="px-4 py-1.5 min-w-8 text-center font-medium">{{ $item['quantity'] }}</span>
                                            <button wire:click="increaseItem({{ $item['product_id'] }})"
                                                class="px-3 py-1.5 hover:bg-amber-50">+</button>
                                        </div>
